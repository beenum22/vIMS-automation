...
public class Algo
{
	...
	void f2345(/* byte[]k, */byte [] rand, byte [] res, byte [] ck, byte [] ik, byte [] ak)
	{
		 ...
		 * XOR on the constant c2 (which * is all zeroes except that the last
		 * bit is 1).
		 */
	...
	*-----------------------------------------------------------------*/
	...
	void f5star( /* byte [] k, */ byte [] rand, byte [] ak)
	{
		...
		Imp.setRijEngine(key, Rij); // this step also computes the working key
									// for object Rij
	 ...
	 * 
	 * String k= "465B5CE8B199B49FAA5F0A2EE238A6BC"; //key String
	 * r="67c6697351ff4aec29cdbaabf2fbe346"; //rand String op=
	 * "1918b840195c97017228127009ca194e"; byte []ck = new byte [16]; byte []ik=
	 * new byte [16]; byte []ak= new byte[6]; byte [] res = new byte [8];
	 * 
	 * //byte[] OP =
	 * {(byte)0x19,(byte)0x18,(byte)0xb8,(byte)0x40,(byte)0x19,(byte)0x5c,(byte)
	 * 0x97,(byte)0x01,(byte)0x72,(byte)0x28,(byte)0x12,(byte)0x70,(byte)0x09,(
	 * byte)0xca,(byte)0x19,(byte)0x4e}; //byte [] key= {(byte)0x46, (byte)
	 * 0x5B,(byte) 0x5C,(byte) 0xE8,(byte) 0x99, (byte)0xB4, (byte)0x9F,
	 * (byte)68, (byte)126, (byte)213, (byte)16, (byte)115, (byte)68, (byte)217,
	 * (byte)58, (byte)108, (byte)56, (byte)218, (byte)5, (byte)78, (byte)28,
	 * (byte)128, (byte)113, (byte)208, (byte)61, (byte)56, (byte)10, (byte)87,
	 * (byte)187, (byte)162, (byte)233, (byte)38 };
	 * 
	 * byte[] key = DatatypeConverter.parseHexBinary(k); byte [] rand
	 * =DatatypeConverter.parseHexBinary(r); byte[]
	 * OP=DatatypeConverter.parseHexBinary(op);
	 * //System.out.print(Byte.toString(key[5]));
	 * //System.out.println(Arrays.toString(key[0])); Algo obj =new
	 * Algo(key,OP); obj.f2345(rand, res, ck, ik, ak); String
	 * RES=DatatypeConverter.printHexBinary(res); System.out.print(RES);
	 * 
	 * 
	 * 
	 * }
