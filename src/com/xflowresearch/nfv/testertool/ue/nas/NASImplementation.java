...
public class NASImplementation
{
	...
	public void NASInit(String AttachReq, String AuthReq, String AuthResp, String AuthRej, String AuthFail,
			String SecureModeCmd, String SecurityModeRej, String SecurityModeComp, String AttachRej, String ESMInfoReq,
			String ESMInfoResp, String AttachComp)
	{
	...
	public String NASAttachRequest(String packetval)
	{
		...
		String thirdByte = NASDefinitions.TypeofSecurityContextFlag.NativeSecurityContext.getHexCode()
				+ NASDefinitions.KeySetIdentifier.NoKeyAvailable.getHexCode()
	...
	public String [] MMEResponseParser(String Response)
	{
		...
		String Output[] =
		{AuthRejPar, AuthFailPar, SecModeRejPar, AuthReqPar, SecModeCmdPar};// parameters
																			// to
																			// be
																			// set
		return Output;
	...
	public String NASSecurityModeComplete(String packetval)
	{
		...
		String Packet = (NASDefinitions.SecurityHeaderType.IntegrityProtectedwithnewEPSSecurityContext.getHexCode()
				...
				+ NASDefinitions.MobilityManagementMessageType.SecurityModeComplete.getHexCode() + value1);
	...
	public String NASESMInformationResponse(String packetval)
	{
		...
		String firstbyte = (NASDefinitions.SecurityHeaderType.IntegrityProtectedCiphered.getHexCode()
				+ NASDefinitions.ProtocolDiscriminatorValue.MobilityManagementMessage.getHexCode()); // first
																									 // BYTE
	...
	public void runNASimulation() throws IOException
	{
		...
		String RES = "634F82417968CA98"; // UE generated Authentication response
										 // parameter information element
										 // parameters : RES value and length of
										 // auth response
		...
		// NASobject.NASInit(AttachReq, AuthReq, AuthRespLength, AuthRej,
		// AuthFail, SecurityModeRej, SecureModeCmd, AttachRej, ESMInfoReq,
		// ESMInfoResp, AttachComplete);//redundant

		AttachReq = NASobject.AttachRequest;// include only the parts not in
											// definitions
		AuthReq = NASobject.AuthRequest;// include whole packet, will be EPC
										// generated, only used at UE for
										// checking
		AuthResp = NASobject.AuthResponse;// include only parts not in
										  // definitons
		AuthRej = NASobject.AuthReject;// complete packet,from MME,for checking
		AuthFail = NASobject.AuthFailure;// complete packet,from MME,for
										 // checking
		 ...
		 * System.out.println(AuthRej); System.out.println(AuthFail);
		 * System.out.println(AuthReq);
		 */// Debugging Comments
		String [] ParserOutput;

		/*
		 * assuming no authentication reject message received.If however an
		 * Authentication Reject message is received, the UE shall abort any EMM
		 * signalling procedure and enter state EMM-DEREGISTERED.
		 */
		...
		if((NASobject.AuthReject.equals("null") && (NASobject.AuthFailure.equals("null")) && !(AuthReq.equals(null))))
		{
			...
			/*
			 * (When MME gets the authentication response message the MME
			 * compares the received RES value with the XRES (Expected Response)
			 * value. If RES == XRES, successful authentication
			 */
		...
		{
			System.out.println("ESM Information Request Received from MME:" + "\n" + "ESM Info Request packet:" + " "
					+ NASobject.ESMInfoRequest);
			NASobject.NASESMInformationResponse(ESMInfoResp); // send ESM
															  // Information
															  // Response
		}
	...
	public String [] respParser(String authReqString)
	{
		...
		String SecurityHeaderType = NASDefinitions.SecurityHeaderType
				.hexToType(hexToBinary(authReqString.substring(0, 1)));// SecurityHeaderType
		// System.out.println(Reply.substring(0,1 )); //debugging

		String ProtocolDiscriminator = NASDefinitions.ProtocolDiscriminatorValue
				.hexToType(hexToBinary(authReqString.substring(1, 2))); // PD
		String EPSMobilityManagementMsg = NASDefinitions.MobilityManagementMessageType
				.hexToType(authReqString.substring(2, 4));// EPS Mobility
														  // Management Message
		String AuthParameterRandValue = null;
		String AUTNvalue = null;

		if((authReqString.substring(2, 4).equals("5c")) || (authReqString.substring(2, 4).equals("5c"))
				|| (authReqString.substring(2, 4).equals("54")))
		{
			AUTNvalue = "N/A";
		...
		String Output[] =
		{SecurityHeaderType, ProtocolDiscriminator, EPSMobilityManagementMsg, AuthParameterRandValue};
