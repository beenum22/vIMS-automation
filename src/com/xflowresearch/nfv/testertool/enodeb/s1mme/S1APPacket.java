 ...
 * S1APPacket
 * 
 * S1APPacket class that allows creation of an S1AP packet given the input
 * parameters and parsing of a received S1AP packet given in a string encoded as
 * hexadecimal.
 * 
 * @author ahmadarslan
 */
public class S1APPacket
{
	...
	public String getValue(String protocolIE)
	{
		...
		{
			if(values.get(i).protocolIE.equals(protocolIE)) return values.get(i).value;
		}
		return null;
	}
	...
	public void insertByteValueLength()
	{
		...
		if(valueLength.length() == 1) valueLength = "0" + valueLength;
	...
	public void insertValues()
	{
		for(int i = 0; i < numOfValues; i++)
		{
			...
			if(ieHexCode.length() == 1) ieHexCode = "000" + ieHexCode;
			if(ieHexCode.length() == 2) ieHexCode = "00" + ieHexCode;
			if(ieHexCode.length() == 3) ieHexCode = "0" + ieHexCode;
			...
			if(valLength.length() == 1) valLength = "0" + valLength;
	...
	public void parsePacket(String packet)
	{
		...
		for(int i = 0; i < numOfValues; i++)
		{
			...
			String lengthOfValueHex = value.substring(a, b); // length greater
															 // than 128... '80'
															 // corner case!!
